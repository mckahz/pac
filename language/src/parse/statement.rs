...
fn let_value(i: &str) -> Result<Statement> {
    ...
    let (i, block) = expression::block.terminated(symbol(";")).parse(i)?;
    ...
    let rhs = if params.is_empty() {
        ...
        params
            .into_iter()
            .fold(Expr::Lambda(last_param, Box::new(block)), |f, arg| {
...
fn let_from(i: &str) -> Result<Statement> {
    let (i, name) = value_identifier.terminated(symbol("<-")).parse(i)?;
    let (i, statements) = many0(parse_statement)(i)?;
    let (i, value) = expression
        .terminated(symbol(";").context("semicolon"))
        .parse(i)?;

    success(Statement::Bind(name, Expr::Block(statements, Box::new(value))))(i)
}
